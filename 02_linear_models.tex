...
\begin{equation}
	\hat \beta = \argmin{\beta_0, \dots , \beta_p} \sum_{i=1}^n \lr{y^{(i)} - \lr{\beta_0 + \sum_{j=1}^p\beta_jx_j^{(i)}}}^{2}
	\label{eq:OLS}
\end{equation}
...
\end{itemize}
\begin{equation}
	\underset{\beta}{min} \frac{1}{n}\sum_{i=1}^n\lr{y^{(i)}-x_i^T \beta }^{2} + \lambda \norm{\beta}_1
	\label{eq:LASSO-L1}
\end{equation}
...
\begin{equation}
	g(\E{Y|x}) = \beta_0 + \beta_1x_1 +  \dots + \beta_p x_p = x^T\beta
...
\begin{itemize}
	...
	\item E.g. Nr of bike Rentals: Temperature and Working/Non-Working day. on days off people only rent bikes when its warm. On working days, people ride to work regardless of weather.
...
\begin{itemize}
	\item Replace features with transformed versions. $log(x), \sqrt{x}, x^2$
...
\begin{itemize}
	...
	\item $f_i(x_i)$ is usually a learned spline function.
	      \begin{itemize}
		      ...
		      \item For each section, a spline is fit e.g cubic $y = ax^3 + bx^2 + cx + d$
